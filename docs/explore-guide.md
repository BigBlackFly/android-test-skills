...
>
> **节级路由**：要开始采 → 「① 真机采集」｜建离线库存 → 「② 离线生成页面库存」
> 写断言前预检 → 「③ 逐句离线预检」｜收尾 → 「④ 真机终验一次」
> 多久算"卡住"、何时该打断人 → 「卡住才找人」｜缓存为什么被删 → 「探查缓存的清理」
>
> 本文件是**操作细则**；"为什么这么设计"的讨论与量化证据在
> [explore-model.md](explore-model.md)（读者是人，AI 不必读）。
