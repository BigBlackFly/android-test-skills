...
1. **解析用例**：从用户消息提取前置条件/操作步骤/预期结果；由 Agent 判断待测应用，并查询 `framework/preparation/app_packages.py` 的 `APP_NAME_PACKAGE_MAP` 获取包名。无法确定应用或查不到包名时，将 `target_package` 设为 `None`，继续测试；其他缺失信息按需澄清。
   **先读 UIUX，再读知识卡**：开始一组用例时，先读 skill 包内 `uiux-knowledge/README.md`，按已确认包名找到 App 索引，经功能索引读取相关页面/跨页主题的 Markdown 正文及适用限制，再进入第 2 步。用宿主文件工具直接读取，同组复用，涉及新页面或上下文丢失时补读。详情 → `docs/uiux-knowledge.md`。
